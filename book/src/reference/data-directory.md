...
|------|-----------|-----------|---------|----------|----------------|
| `BILLS-*.xml` | For extraction | `download` | `extract`, `upgrade`, `audit` | Never | ~1.8 MB |
...
- Without `BILLS-*.xml`: Extraction and upgrade can't run (they need the source text). Query commands work fine.
- Without `embeddings.json` + `vectors.bin`: `--semantic` and `--similar` searches are unavailable.
